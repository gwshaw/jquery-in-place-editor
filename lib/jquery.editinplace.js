...
Modified:
- to include additional bug/compatibility fixes from bug tracker.
- to not display select_text message if it is empty.
- to supply initial select option from initial value
- to instantiate settings for each EIP instance
- to load HTML5 data- options to each instance
- to support asynchronous Ajax in the callback
- to allow editor to stay in place on an application reported error
  (such as data validation)
- remove .browser check as it no longer exists
- change 'bind' and 'unbind' to 'on' and 'off' as preferred usage
- trigger custom event when initialization completed
...
$.fn.editInPlace = function(options) {
  ...
  // Modification
  // moved var settings and related code to below so it is instantiated for each editor,
  // not just once for each selector.

  return this.each(function() {
    var dom = $(this);
    // Modification
    // Move check to top--why parse variables if we won't use them?
    ...
    // Modification
    // add ability to collect instance parameters from HTML5 data directive
    var dataEIP = eval("dataEIP=(" + dom[0].getAttribute("data-EIP") + ")");
    var settings = $.extend({}, $.fn.editInPlace.defaults, options, dataEIP);
    ...
    // Modification
    // Tell the world we're all done so editor can be program-clicked to open
    dom.trigger('bound.editInPlace');
  });
};
...
$.extend(InlineEditor.prototype, {
  ...
  connectOpeningEvents: function() {
    var that = this;
    this.dom
      .on('mouseenter.editInPlace', function(){ that.addHoverEffect(); })
      .on('mouseleave.editInPlace', function(){ that.removeHoverEffect(); })
      .on('click.editInPlace', function(anEvent){ that.openEditor(anEvent); });
  },

  disconnectOpeningEvents: function() {
    // prevent re-opening the editor when it is already open
    this.dom.off('.editInPlace');
  },
  ...
  setInitialValue: function() {
    ...
    // Modification
    // if a select tag with values, then get the value rather than the name.
    if (this.settings.field_type ==="select" && this.settings.select_options[0][1] !== undefined)
    {
      for(i in this.settings.select_options) {
        if (this.settings.select_options[i][0]===this.originalValue)
        {
          initialValue = this.settings.select_options[i][1];
          break;
        }
      }
    }
    ...
    // Modification
    // Below fixed for not displaying in some browsers per below.
    // http://code.google.com/p/jquery-in-place-editor/issues/detail?id=101
  ...
  createSelectEditor: function() {
    var i;
    // Modification
    // Don't add a blank disabled option if selected_text is empty.
  ...
  // Modification
  // Fix applied below for bug of not restoring original value on cancel.
  // Applied from r161 in svn repository but not included on download page.
  // http://code.google.com/p/jquery-in-place-editor/source/browse/trunk/lib/jquery.editinplace.js?r=161
  ...
  handleSubmitToCallback: function(enteredText) {
    ...
    // Modified
    // Supply this.settings instead of this.settings.params to be able to
    // superset the internal Ajax call in the callback.
    // Modified
    // Adds a function parameter as resultCallback to allow asynchronous Ajax, and recoded
    // call structure to use it.
    that = this;
    this.triggerCallback(this.settings.callback, /* DEPRECATED in 2.1.0 */ this.id(),
      enteredText, this.originalValue, this.settings, this.savingAnimationCallbacks(),
      function(newHTML) {
        if (that.settings.callback_skip_dom_reset) {
          // Modification
          // If callback returns undefined then return focus to input and
          // stay in editor. This typically means the input did not pass
          // validation at the server. Otherwise exit the editor.
          if (undefined === newHTML) {
            var ipeField = that.dom.find('.inplace_field');
            ipeField.focus();
            ipeField.val(enteredText);
            return;  // keep editor intact
        ...
        else
        {
          ...
          that.dom.html(newHTML);
        }

        if (that.didCallNoCallbacks()) {
          that.enableOrDisableAnimationCallbacks(false, false);
          that.reinit();
        }
      }
    );
  ...
  startSavingAnimation: function() {
    ...
    this.dom
      ...
      .animate({ backgroundColor: 'transparent'}, 400, 'swing', function(){
        ...
        // Hence we need to prevent this from becoming an unbounded recursion.
...
function assertMandatorySettingsArePresent(options) {
  ...
  throw new Error("Need to set either url: or callback: option for the in-line editor to work.");
}
